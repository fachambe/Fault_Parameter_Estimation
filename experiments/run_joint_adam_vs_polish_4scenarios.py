# experiments/run_joint_adam_vs_polish_4scenarios.py
"""
Compare Adam, Adam + BFGS, AdaM + LM, Adam + Newton for L1, ZF, ZL across 4 scenarios:
...
def main():
    ...
    for scenario in SCENARIOS:
        ...
        # Est1: Adam only, Est2: Adam + Newton, Est3: Adam + LM, Est4: Adam + BFGS
        ...
        rmse_L1_bfgs_list, rmse_ZF_bfgs_list, rmse_ZL_bfgs_list = [], [], []
        crlb_L1_list = []
        crlb_ZF_list = []
        crlb_ZL_list = []
        ...
        for snr_db in SNR_DBS:
            ...
            # More Adam steps for Low_Freq_Short_Cable (harder scenario)
            adam_steps = 20000 if scenario_name == "Low_Freq_Short_Cable" else 2000
            ...
            # Est4: Adam + BFGS
            rmse_L1_bfgs = float(np.sqrt(np.mean((preds4["L1"] - data["L1_true"]) ** 2)))
            rmse_ZF_bfgs = rmse_complex(preds4["ZF"], ZF_true)
            rmse_ZL_bfgs = rmse_complex(preds4["ZL"], ZL_true)
            rmse_L1_bfgs_list.append(rmse_L1_bfgs)
            rmse_ZF_bfgs_list.append(rmse_ZF_bfgs)
            rmse_ZL_bfgs_list.append(rmse_ZL_bfgs)
            ...
            print(f"    L1: Adam={rmse_L1_adam:.4f}, Newton={rmse_L1_newton:.4f}, LM={rmse_L1_lm:.4f}, BFGS= {rmse_L1_bfgs:.4f}, sqrt(CRLB)={sqrt_crlb_L1:.4f}")
            print(f"    ZF: Adam={rmse_ZF_adam:.4f}, Newton={rmse_ZF_newton:.4f}, LM={rmse_ZF_lm:.4f}, BFGS= {rmse_ZF_bfgs:.4f}, sqrt(CRLB)={sqrt_crlb_ZF:.4f}")
            print(f"    ZL: Adam={rmse_ZL_adam:.4f}, Newton={rmse_ZL_newton:.4f}, LM={rmse_ZL_lm:.4f}, BFGS= {rmse_ZL_bfgs:.4f}, sqrt(CRLB)={sqrt_crlb_ZL:.4f}")
        ...
        all_results[scenario_name] = {
            ...
            # Adam + BFGS
            "rmse_L1_bfgs": np.array(rmse_L1_bfgs_list),
            "rmse_ZF_bfgs": np.array(rmse_ZF_bfgs_list),
            "rmse_ZL_bfgs": np.array(rmse_ZL_bfgs_list),
            # CRLB
    ...
    results_file = results_dir / f"joint_adam_vs_polish_4scenarios_seed{SEED}.npz"
    ...
    for name, results in all_results.items():
        ...
        # Adam + BFGS
        save_dict[f"{name}_rmse_L1_bfgs"] = results["rmse_L1_bfgs"]
        save_dict[f"{name}_rmse_ZF_bfgs"] = results["rmse_ZF_bfgs"]
        save_dict[f"{name}_rmse_ZL_bfgs"] = results["rmse_ZL_bfgs"]
        # CRLB
    ...
    print(f"Run plot_joint_adam_vs_polish_4scenarios.py to generate figures.")
