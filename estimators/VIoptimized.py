...
class VIoptimized(Estimator):
    ...
    def guide(self, y_ri, sig_f):
        ...
        with pyro.plate("obs", M):
            ...
            L1_scale = pyro.param("L1_scale", torch.full((M,), self.fixed_sigma, device=device),
    ...
    # Get per-run ELBO vector [M] in one pass
    def _per_run_elbo_once(self, guide_fn, y_ri, sig_f, num_particles):
        ...
        def keep_obs_only(t):
            ...
            # Right now all log_probs return [M] so not needed technically but good practice to do
            if t.dim() <= 1:
                return t
    ...
    def guide_L1_mu_Z_delta(self, y_ri, sig_f, mu_vec, uZ):
        """
        y_ri:  [M,F,2]
        ...
        ZF_im = self.ImZF_max * torch.tanh(u2)
        ZL_re = self.ReZL_lo + (self.ReZL_hi - self.ReZL_lo) * torch.sigmoid(u3)
        ZL_im = self.ImZL_max * torch.tanh(u4)
        ...
        with pyro.plate("obs", M):
            ...
            # Z is deterministic but differentiable wrt uZ
            pyro.sample("ZF_re", dist.Delta(ZF_re))
            pyro.sample("ZF_im", dist.Delta(ZF_im))
            pyro.sample("ZL_re", dist.Delta(ZL_re))
            pyro.sample("ZL_im", dist.Delta(ZL_im))

    def _per_run_elbo_once_grad(self, guide_fn, y_ri, sig_f, num_particles):
        M = y_ri.size(0)
        device = y_ri.device
        elbo_acc = torch.zeros(M, device=device)
        ...
        def keep_obs_only(t):
            return t if t.dim() <= 1 else t.sum(dim=tuple(range(1, t.dim())))
        ...
        for _ in range(num_particles):
            ...
            for s in mt.nodes.values():
                if s["type"] == "sample" and under_obs(s):
                    vec = vec + keep_obs_only(s["fn"].log_prob(s["value"]))
            ...
            elbo_acc = elbo_acc + vec

        return elbo_acc / float(num_particles)  # [M]
    ...
    def _profile_L1_mu_grid(self, y_ri, sig_f):
        ...
        for g, mu in enumerate(mu_grid):
            ...
            elbo_per_run[g] = self._per_run_elbo_once(guide_mu, y_ri, sig_f, num_particles=150)
            if g % 50 == 0:
